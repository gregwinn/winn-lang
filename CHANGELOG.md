...
### Fixed
- **HTTP client request timeouts are now configurable** ([#184](https://github.com/gregwinn/winn-lang/issues/184)) — `winn_http` inherited hackney's 8s-connect / 5s-receive defaults with no override, so valid-but-slow endpoints failed with `{error, timeout}`. Each verb takes an optional trailing options map (`HTTP.get(url, %{timeout: 30000})`) supporting `timeout` / `recv_timeout` / `connect_timeout` (ms) and `follow_redirect`; defaults raised to connect 15s / recv 30s. Backward compatible — original arities retained. Also shipped as the 0.9.4 hotfix off the release line.
