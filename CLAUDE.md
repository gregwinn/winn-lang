...
Commands: `new`, `compile`, `run`, `start`, `test`, `docs`, `watch`, `create`/`c`, `task`, `migrate`, `rollback`, `release`, `bench`, `metrics`, `add`, `remove`, `packages`, `install`, `console`, `deps`, `version`, `help`. The `start` command compiles all `src/*.winn`, loads `_build` dep paths, starts OTP apps, calls `main()`, and blocks with `receive` to keep the VM alive. The `run` command reads the module name from the source file (regex on `module Name`), not from the filename.
